...
from benchmark.fake_tools import ToolResult, get_tool
...
# ---------------------------------------------------------------------------
# Live runner: DeepSeek API + real sandbox tool execution
# ---------------------------------------------------------------------------
...
def run_scenario_live(
    ...
    """Run a scenario with a real LLM provider and real tool executor.

    Supports DeepSeek API + local/remote sandbox while still enforcing
    all guards (RuntimeGuard, ToolFirewall, ArgumentGuard).

    Args:
        scenario: A fully loaded ScenarioSpec.
        output_dir: Optional directory for trace output.
        sandbox_root: Override sandbox root (for local_sandbox executor).

    Returns:
        Dict with scenario_id, status, failure_type, passed, trace_file, etc.
    """
    from benchmark.providers.factory import create_provider
    ...
    # Create provider and executor from scenario config
    provider = create_provider(scenario.agent_run)
    sandbox = scenario.agent_run.sandbox
    if sandbox is not None and sandbox_root is not None:
        sandbox_root = Path(sandbox_root)
    elif sandbox is not None:
        sandbox_root = sandbox.cwd or ".tmp/live-sandbox"
    else:
        sandbox_root = Path(".tmp/live-sandbox")
    executor = LocalSandboxToolExecutor(
        sandbox_root=sandbox_root,
        timeout_seconds=sandbox.timeout_seconds if sandbox else 10,
        max_output_bytes=sandbox.max_output_bytes if sandbox else 20000,
        network_allowlist=sandbox.network_allowlist if sandbox else [],
        command_allowlist=sandbox.command_allowlist if sandbox else [],
    )
    ...
    tool_schemas = get_tool_schemas(scenario.agent_run.tools)

    provider_name = scenario.agent_run.provider
    model_name = scenario.agent_run.model
    executor_name = scenario.agent_run.tool_executor
    ...
    model_decision = guard.check_model(model_name, policy)
    _emit(EventType.guard_decision, "guard", {
        "action": model_decision.action.value,
    ...
    messages: list[dict[str, Any]] = [
        ...
        {"role": "user", "content": scenario.agent_run.task},
    ]
    ...
    for _ in range(scenario.agent_run.max_steps):
        ...
        # llm_request
        _emit(EventType.llm_request, provider_name, {
            "provider": provider_name,
            "model": model_name,
        ...
        # Call the LLM
        try:
            response = provider.chat(messages, tools=tool_schemas if tool_schemas else None)
        except Exception as exc:
            _emit(EventType.llm_response, provider_name, {
                "error": str(exc),
            }, error=str(exc))
            _emit(EventType.agent_end, "runner", {"status": "error"})
            return _finalise(logger, scenario, run_id, "error", trace_path)

        used_tokens += response.total_tokens

        # llm_response — content preview only, never log full raw
        ...
        # If no tool calls and content → treat as final answer
        if not response.tool_calls and response.content:
            _emit(EventType.final_answer, "runner", {
                "final_answer": content_preview,
                "tool_evidence_count": len(tool_call_history),
            })

            # Answer verification check
            if policy.require_answer_verification:
                verified = len(tool_call_history) > 0
                av_decision = guard.check_final_answer_verified(verified, policy)
                _emit(EventType.guard_decision, "guard", {
                    "action": av_decision.action.value,
        ...
        for tool_call_index, tc in enumerate(response.tool_calls):
            trace_args = _trace_arguments_from_arguments(tc.arguments)
            tool_call_id = tc.call_id or f"call_{step}_{tool_call_index}_{tc.tool}"

            # Duplicate detection
            call_sig = (tc.tool, dict(tc.arguments))
            is_dup = call_sig in tool_call_history
            ...
            result: ToolResult = executor.execute(tc.tool, tc.arguments)
            exec_ms = int((_time.monotonic() - exec_start) * 1000)

            # Build tool result trace data
            ...
            if isinstance(result.output, dict):
                tool_result_data["exit_code"] = result.output.get("exit_code")
                stdout = result.output.get("stdout", "")
                stderr = result.output.get("stderr", "")
                tool_result_data["stdout_preview"] = str(stdout)[:200]
                tool_result_data["stderr_preview"] = str(stderr)[:200]
            else:
                tool_result_data["output_preview"] = str(result.output)[:200] if result.output else ""

            _emit(EventType.tool_result, "tools", tool_result_data, error=result.error)
            ...
            tool_output_str = ""
            if isinstance(result.output, dict):
                tool_output_str = json.dumps(result.output, ensure_ascii=False)
            elif result.output is not None:
                tool_output_str = str(result.output)
            if result.error:
                tool_output_str = f"Error: {result.error}"
